...
void CanvasPicker::select( const QPoint &pos )
{
          ...
          it != itmList.end(); ++it )
    {
        if ( ( *it )->rtti() == QwtPlotItem::Rtti_PlotCurve &&
             (static_cast<QwtPlotCurve *>( *it ))->style() == QwtPlotCurve::Lines &&
             (static_cast<QwtPlotCurve *>( *it ))->isVisible() &&
             (static_cast<QwtPlotCurve *>( *it ))->title().text() != "相对均方误差")
        {
...
void CanvasPicker::showCursor( bool showIt )
{
    ...
    if( d_selectedCurve->title().text() == "相对均方误差")
    {
        return;
    }
...
void CanvasPicker::shiftCurveCursor( bool up )
{
    ...
    showCursor( false );
    d_selectedPoint = 0;//这个要开启，万一相邻的两条线上的点数不一样，那就是outindex
